...
{
    uint16_t val = ((value & 0xFF) << 8) + (value >> 8);
    i2c_write(&_i2cDevice, reg, &val, 2);
}
...
void Adafruit_INA219::setCalibration_32V_2A(void) {
    ...
    // VSHUNT_MAX = 0.32          (Assumes Gain 8, 320mV, can also be 0.16,
    // 0.08, 0.04) RSHUNT = 0.1               (Resistor value in ohms)
