...
SE3 compute_T_vane_unit_from_aerodynamic_frame() {
  ...
  const SO3 R_intermediate_from_aerodynamic_frame = SO3::exp(jcc::Vec3(0.0, M_PI * 0.5, 0.0));
  const SO3 R_vane_unit_from_intermediate = SO3::exp(jcc::Vec3(0.0, 0.0, -M_PI * 0.5));
  const SO3 R_vane_unit_from_aerodynamic_frame = R_vane_unit_from_intermediate * R_intermediate_from_aerodynamic_frame;

  const SE3 T_vane_unit_from_zero = SE3(R_vane_unit_from_aerodynamic_frame, vane_center_of_pressure_vane_unit_frame);
...
Wrench vane_unit_wrench(const double commanded_servo_angle_rad,
  ...
  // commanded_vane_angle_rad is the angle of the vane chord-line w.r.t the vehicle axis of symmetry
  const double commanded_vane_angle_rad =
      (commanded_servo_angle_rad / vane_cfg.gear_ratio_servo_from_vane) + vane_cfg.offset_vane_from_servo_rad;
  ...
  const double lift_force_at_vane_N = dynamic_pressure_Pa * lift_coeff * vane_cfg.vane_area_m2;

  const double drag_coeff =
      vane_cfg.cd0 + (4.0 * M_PI * M_PI * vane_cfg.k * (angle_of_attack_rad * angle_of_attack_rad));
  const double drag_force_N = dynamic_pressure_Pa * drag_coeff * vane_cfg.vane_area_m2;
  assert(drag_force_N >= 0.0);

  const jcc::Vec2 force_exhaust_velocity_frame(drag_force_N, lift_force_at_vane_N);

  // The vane "unit" frame; as described in the CAD
  const SO2 vane_unit_from_exhaust_velocity = SO2::exp(SWIRL_ANGLE);
  const jcc::Vec2 force_vane_aerodynamic_frame = vane_unit_from_exhaust_velocity * force_exhaust_velocity_frame;
  const jcc::Vec3 force_vane_aerodynamic_frame_3d = jcc::augment(force_vane_aerodynamic_frame, 0.0);

  const Wrench wrench_aerodynamic_frame({.force_N = force_vane_aerodynamic_frame_3d,  //
                                         .torque_Nm = jcc::Vec3::Zero()});

  const Wrench wrench_vane_unit_frame = wrench_in_frame(vane_cfg.T_vane_unit_from_zero, wrench_aerodynamic_frame);
