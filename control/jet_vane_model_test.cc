...
void test_force_monotonic_in_throttle() {
  ...
  vane_cfg.T_vane_unit_from_zero = SE3();
  ...
  const Wrench wrench_zero_throttle = vane_unit_wrench(commanded_servo_angle_rad, status_zero, vane_cfg, jet_cfg);
  const Wrench wrench_half_throttle = vane_unit_wrench(commanded_servo_angle_rad, status_half, vane_cfg, jet_cfg);
  const Wrench wrench_full_throttle = vane_unit_wrench(commanded_servo_angle_rad, status_full, vane_cfg, jet_cfg);
...
void test_torque_non_identity_offset() {
  ...
  VaneConfiguration vane_cfg;
  vane_cfg.T_vane_unit_from_zero = SE3(SO3(), jcc::Vec3(0.0, 0.0, -1.0));
