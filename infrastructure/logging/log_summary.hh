...
struct LogSummary {
  ...
  LogSummary(const std::string log_path) {
    ...
    for (std::size_t i = 0; i < channels.size(); ++i) {
      ...
      // TODO(mason): Refactor to not assign last message received every time
  ...
  void print() {
    // Output Header in Bold Red
    const std::string reset("\033[0m");
    const std::string red_bold("\033[31;1m");
    std::cout << red_bold << std::setw(30) << "Channel" 
                          << std::setw(25) << "First Timestamp" 
                          << std::setw(25) << "Last Timestamp" 
                          << std::setw(17) << "Duration (s)" 
                          << std::setw(15) << "Num Messages" 
                          << std::setw(17) << "Frequency (Hz)" 
                          << std::endl;
    std::cout << reset;
    ...
    for(auto it = channel_stats.begin(); it != channel_stats.end(); it++) {
      ...
      std::cout << std::setw(30) << it->first << 
                   ...
                   std::setw(17) << std::fixed << std::setprecision(3) << it->second.frequency << std::endl;
    }
  }
};
