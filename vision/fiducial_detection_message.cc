...
std::optional<FiducialDetectionMessage> create_fiducial_detection_message(const cv::Mat& camera_frame,
  ...
  const std::optional<SE3> board_from_camera = estimate_board_bottom_left_from_camera(ids_corners, camera_calibration);
  if (board_from_camera) {
    ...
    const std::vector<BoardPointImagePointAssociation> board_point_assocs =
        obj_points_img_points_from_image(ids_corners);
