...
Eigen::Matrix4f get_perspective_mat(double fov, double aspect, double near,
                                    double far) {
  double yScale = 1.0 / tan(fov);
  double xScale = yScale / aspect;
  Eigen::Matrix4f mat;
  mat << xScale, 0, 0, 0, 0, yScale, 0, 0, 0, 0, -(far + near) / (near - far),
      -2 * far * near / (near - far), 0, 0, -1, 0;

  return mat;
}

Eigen::Matrix4f get_camera_from_world(Eigen::Vector3f camera_loc,
  ...
  camera_from_world_rotation << right.x(), right.y(), right.z(), 0, up.x(),
      up.y(), up.z(), 0, look_direction.x(), look_direction.y(),
      look_direction.z(), 0, 0, 0, 0, 1;

  Eigen::Matrix4f camera_from_world_translation;
  camera_from_world_translation << 1, 0, 0, -camera_loc.x(), 0, 1, 0,
      -camera_loc.y(), 0, 0, 1, -camera_loc.z(), 0, 0, 0, 1;
  return camera_from_world_rotation * camera_from_world_translation;
}

void enter_vertex_buffer_state(GLuint vertex_buffer_id,
