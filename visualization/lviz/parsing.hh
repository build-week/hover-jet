...
std::regex ALL_COLOR_NAMES_REGEX(ALL_COLOR_NAMES_STR, std::regex::optimize);
...
std::regex NEW_FRAME_REGEX("new_frame", std::regex::optimize);
std::regex SET_FRAME_TIME_REGEX("set_frame_time" + WHITESPACE_STR + "(" + FLOAT_STR + ")", std::regex::optimize);

std::regex WHITESPACE_REGEX(WHITESPACE_STR, std::regex::optimize);

rgb_color parse_color(const std::string &color_string) {
  std::smatch matches;
  if (std::regex_match(color_string, matches, RGB_COLOR_REGEX)) {
    return rgb_color{stof(matches[1]) / 255.0f, stof(matches[2]) / 255.0f, stof(matches[3]) / 255.0f};
  }

  else

      if (std::regex_match(color_string, matches, COLORMAP_COLOR_REGEX)) {
